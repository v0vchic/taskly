...
export const ProjectItem = ({
  ...
  // ✅ Only update state if title actually changed to prevent cascading renders
  useEffect(() => {
        ...
        ? (
            <div className="flex items-center gap-1 px-1 py-1">
              <input
                ...
                onBlur={confirm}
                className="flex-1 min-w-0 text-xs text-white bg-white/10 border border-white/25
              rounded-lg px-2 py-1.5 outline-none focus:border-white/50"
              />
              <button
                onMouseDown={(e) => {
                  e.preventDefault()
