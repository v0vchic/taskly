...
export const useAppState = () => {
  ...
  const setActiveProjectId = (id: string) => setState(prev => ({ ...prev, activeProjectId: id }))
  ...
  const addProject = (title: string, color: string) => {
    ...
    setState(prev => ({ projects: [...prev.projects, project], activeProjectId: id }))
  }

  const renameProject = (id: string, title: string) =>
    setState(prev => ({ ...prev, projects: prev.projects.map(p => p.id === id ? { ...p, title } : p) }))

  const deleteProject = (id: string) =>
    setState((prev) => {
      const remaining = prev.projects.filter(p => p.id !== id)
      return { projects: remaining, activeProjectId: prev.activeProjectId === id ? remaining[0].id : prev.activeProjectId }
    })

  return { state, activeProject, setActiveProjectId, updateActiveProject, addProject, renameProject, deleteProject }
}
